    ...
    def collect(
        ...
        for phase_name, runs in runs_by_phase.items():
            ...
            for run in runs:
                ...
                for turn in run.turns:
                    ...
                    if turn.validation.status == ValidationStatus.VALID:
                        ...
                        if (
                            turn.concern
                            and "직접적 리스크" not in turn.concern
                            and "모니터링" not in turn.concern
                        ):
            ...
            phase_score = PhaseScore(
                ...
                observed_risk_categories=list({i.issue_category for i in detected_issues}),
        ...
        logger.info(
            ...
            len(phase_logs),
            sim_log.total_issues,
            sim_log.total_actions,
            sim_log.total_unresolved,
        )
        return sim_log
...
def _build_summary(
    ...
    if issue_cats:
        summary_parts.append(f"관찰된 issue 카테고리: {', '.join(issue_cats)}.")
    if high_issues:
    ...
    if unresolved:
        summary_parts.append(f"evidence 미확보 발언 {len(unresolved)}건이 unresolved로 기록됨.")
