...
class TeamSimulationLog(BaseModel):
    ...
    def model_post_init(self, __context) -> None:
        object.__setattr__(self, "total_issues",    sum(len(p.detected_issues) for p in self.phase_logs))
        object.__setattr__(self, "total_actions",   sum(len(p.action_items) for p in self.phase_logs))
        object.__setattr__(self, "total_unresolved",sum(len(p.unresolved_questions) for p in self.phase_logs))
