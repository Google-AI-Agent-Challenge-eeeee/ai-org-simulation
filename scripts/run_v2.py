...
evidence_list = [
    ...
    for e in json.loads(
        (SAMPLES / "sample_evidence_metadata_v2.json").read_text(encoding="utf-8")
    )
]
...
invalid_turns = sum(
    1 for r in orch_out.phase_runs
    for t in r.turns if not t.validation.status == "valid"
)
...
print(f"  HIGH/MED/LOW : {issue_summary.high_count}/{issue_summary.medium_count}/{issue_summary.low_count}")
...
for d in breakdown.dimensions:
    ...
    print(f"  {d.dimension:<25} {d.raw_score:.3f}  [{bar}]  {d.status if hasattr(d, 'status') else ''}")
